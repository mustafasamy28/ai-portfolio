...
/**
 * Call OpenAI API to generate response (same as chat.js)
 */
...
/**
 * Main handler for /api/voice-chat endpoint
 */
export default async function handler(req, res) {
  ...
  try {
    // Parse FormData
    // Note: Vercel serverless functions need proper FormData parsing
    // The frontend sends FormData via axios, which Vercel should handle
    // We'll parse it from the request body
    
    // For Vercel, we need to handle multipart/form-data
    // Since axios sends FormData, we'll expect it in req.body
    // In production, you may need to install 'formidable' or 'busboy' for proper parsing
    // For now, we'll use a simplified approach that works with Vercel's runtime
    
    const audioFile = req.body?.audio_file || req.body?.file;
    const sessionId = req.body?.sessionId;
    const messageCount = parseInt(req.body?.messageCount || '0', 10);
    ...
    try {
      ...
      return res.status(500).json({
        ...
        message:
          'Sorry, I could not transcribe your voice message. Please try again or use text chat.',
      });
    }
